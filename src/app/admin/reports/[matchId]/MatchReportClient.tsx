...
export default function MatchReportClient({ match }: { match: any }) {
  ...
  return (
    <div className="w-full flex flex-col gap-6 animate-fade-in pb-24 max-w-lg mx-auto">
      ...
      {/* ======= HAFTANIN ANALİZCİLERİ (EN İYİ VE EN KÖTÜ) ======= */}
      <div className="flex flex-col gap-3">
        ...
        {worstAnalyst && worstAnalyst.voterId !== analyst?.voterId && (
          <div className="bg-gradient-to-r from-rose-600 to-red-700 rounded-3xl p-5 text-white shadow-xl shadow-rose-500/20 relative overflow-hidden flex items-center justify-between">
            <div className="absolute -right-6 -bottom-6 w-32 h-32 bg-white/10 rounded-full blur-2xl pointer-events-none"></div>
            
            <div className="flex items-center gap-3.5 z-10">
              <div className="w-12 h-12 bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl flex items-center justify-center text-rose-200 shadow-inner">
                <AlertTriangle size={24} />
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase text-rose-200 tracking-widest">En Kötü Tahminci (Karavana)</span>
                <span className="text-lg font-black text-white leading-tight mt-0.5">{worstAnalyst.name}</span>
                <span className="text-[11px] text-rose-100 font-medium mt-0.5">{worstAnalyst.score} Oyuncuda En Uzak Tahmin</span>
              </div>
            </div>
